---
title: "Greece - Why do Young People keep leaving? 🧠✈️"
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> Notes 📝 </p>

<p style="text-align: justify;">
I will use data from the [OECD organization](https://www.oecd.org/). The Organization for Economic Co-operation and Development (OECD) is a unique forum where the governments of 37 democracies with market-based economies collaborate to develop policy standards to promote sustainable economic growth. 
</p>
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> OECD Data Categories 📊 </p>
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> Unemployment 📉 </p>
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> Employment 💼 </p>
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> Productivity 📅 </p>
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> Wages 💶 </p>
...
```{r, echo = F}
# Keep Greece, Slovakia, Hungary, Spain, Italy
average_wage_summary <- wage_average %>% 
  filter(Country %in% c("Greece", "Slovakia" , "Hungary" , "Spain", "Italy")) %>%
  filter(SUBJECT =="TOT")
  
# Plot
average_wage_summary %>%
  ggplot( aes(x=TIME, y=Value, group=Country, color=Country)) +
    geom_line() +
    geom_point() +
    scale_color_viridis(discrete = TRUE) +
    ggtitle("Average Wages (USD) 1995-2020") +
    theme_ipsum() +
    ylab("Average Wage in USD") 
   # transition_reveal(TIME)
```
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> Population - Demography 🧍 </p>
...
gdp_debt <- vroom("GDP_Economy/General_Government_debt.csv", col_types = cols())
# Getting view of the data
...
# filtering the Countries we will use and Keeping only the Data that are Annual Frequency and MEASURE as USD per Capita
gdp_debt <- gdp_debt[gdp_debt$LOCATION %in% c("AUS","AUT", "BEL", "CAN", "CZE", "DNK", "FIN", "FRA", "DEU", "GRC", "HUN", "IRL", "ITA","LUX","NLD", "NOR", "POL", "PRT", "SVK", "ESP", "SWE", "CHE", "GBR", "USA", "EST", "SVN", "LVA", "LTU") & gdp_debt$FREQUENCY == "A", ]
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> GDP - Debt 💰 </p>
...
```{r GDP Government Debt Chart, echo = F}
# Plot General Government Debt for 2020
gdp_chart_debt <- gdp_debt[gdp_debt$TIME == 2020 & gdp_debt$SUBJECT == "TOT",]
gdp_chart_debt <- gdp_chart_debt %>% 
  arrange(-Value) 


gdp_chart_debt %>% 
 hchart(type = 'column', options3d = list(enabled = TRUE, alpha = 15, beta = 15), hcaes(x = reorder(Country,Value), y = Value), color = "#d3d3d3", name = "% of Total GDP") %>% 
 hc_yAxis(title = list(text = "% of Total GDP")) %>%  
 hc_xAxis(title = list(text = "Country")) %>% 
 hc_title(text = "General Government Debt, <b> Total % in 2020</b>", style = list(color = "#FF0000", useHTML = TRUE)) %>% 
 hc_plotOptions(series = list(stacking = FALSE, allowPointSelect = TRUE)) %>%
  hc_chart(events = list(load = JS("function() {
        let chart = this,
          x;
...
# Custom arrange for the order I want in the table
greece_summary <- greece_summary %>% 
  mutate(order = c(1:9)) %>% 
  arrange(match(order, c(8,9,4,3,1,5,2,6,7)))
```
...
gt_all <- all_summary %>% 
  dplyr::select(Country, Flags, `Unemployment Rate`, `Long-term Unemployment Rate`, `Average Wage`, `Hours Worked`, `GDP per hour worked`, GDP) %>% 
  gt() %>% 
  tab_header(
    title = md("**Summary - Key Country Metrics**"),
    subtitle = md("An overview of Country OECD data for the year **2020**")
  ) %>% 
   cols_align(
      align = c("center"),
  ...
  cols_label(
     Flags = " ",
     `Unemployment Rate` = md("**Unemployment Rate,<br>ratio in %**"),
     `Average Wage` = md("**Average Wage,<br>in USD**"),
     `Hours Worked` = md("**Hours Worked,<br> Annual Hrs**"),
     `GDP per hour worked` = md("**GDP/hour worked,<br>in USD**"),
     GDP = md("**GDP,<br> USD per capita**"),
      `Long-term Unemployment Rate`= md("**Long-term Unemployment,<br>ratio in %**")) %>%  
  fmt(
    columns = `Long-term Unemployment Rate`,
    fns = function(x) ifelse(x == 0, " — ", all_summary$`Long-term Unemployment Rate`)) %>% 
  fmt_currency(
    columns = vars(`Average Wage`, `GDP per hour worked`, GDP ),
    currency = "USD",
    suffixing = TRUE
  ) %>% 
  fmt_percent(
    columns = vars(`Unemployment Rate`, `Long-term Unemployment Rate`),
    scale_value = FALSE
  ) %>% 
  opt_table_font(
    font = list(
  ...
  tab_source_note(source_note = md("[**OECD org**](https://www.oecd.org/) 2020 Data ")) %>%
  tab_footnote(
    footnote = "Grey fields for data that are not updated from OECD.org  for 2020",
    locations = cells_column_labels(columns = `Long-term Unemployment Rate`)
  ) %>% 
...
gt_greece <- greece_summary %>% 
  dplyr::select(category, Value, average) %>% 
  gt() %>% 
  tab_header(
    title = md("**Summary Greece & Average 2020**"),
    subtitle = "An overview of OECD 2020 data"
  ) %>%  
   cols_align(
      align = c("center"),
      columns = c(everything())
  ...
  tab_source_note(source_note = md("[**OECD org**](https://www.oecd.org/) 2020 Data")) %>%
  tab_footnote(
    footnote = "Countries included for the calculation of the average are the ones used for the whole analysis",
    locations = cells_column_labels(columns = average)
  ) %>% 
  tab_style(
...
<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> End 🏁 </p>


<p style="background-color:#001489 ;color:#FFFFFF ;font-size:25px;text-align:center;border-radius:10px 10px;font-weight:bold;border-style:solid;border-color:#001489;"> References 📚 </p>
